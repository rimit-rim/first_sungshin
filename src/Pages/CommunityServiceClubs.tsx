...
const allClubs: { [key: string]: Club[] } = {
  'Performance': [],
  'Community Service': [ 
    { name: 'Sungshin RCY', description: 'Sungshin Red Cross Youth', subDescription: "Volunteers service in children and teenagers education, multicultural community.", instagram: 'https://www.instagram.com/rcy_sswu/' },
  ],
  'Art': [],
  'Religion': [],
  'Hobby': [],
...
const ClubCard: React.FC<{ club: Club }> = ({ club }) => (
    <div className="flex flex-col sm:flex-row items-center w-full">
      <div className="bg-white w-40 h-28 rounded-xl flex items-center justify-center mb-4 sm:mb-0 sm:mr-6 flex-shrink-0">
        <span className="font-semibold text-gray-700 text-2xl text-center">{club.name}</span>
      </div>
...
// 메인
export default function CommunityServiceClubs() {
  const [isSidebarOpen, setSidebarOpen] = useState(false);

  const clubsToShow = allClubs['Community Service']; 
      ...
      <main className="px-4 sm:px-6 md:px-10 xl:px-[100px] my-8">
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <section className="w-full md:w-3/4 bg-violet-100 p-4 sm:p-6 rounded-2xl">
            <h2 className="text-xl font-bold text-gray-800 mb-6">Clubs in Community Service Department</h2>
